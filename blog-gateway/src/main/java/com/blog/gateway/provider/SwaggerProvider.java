package com.blog.gateway.provider;
...
public class SwaggerProvider implements SwaggerResourcesProvider {
    ...
    public List<SwaggerResource> get() {
        ...
        gatewayProperties.getRoutes().stream()
                .filter(rd -> routes.contains(rd.getId()))
                .forEach(rd -> rd.getPredicates().stream()
                        .filter(pd -> "Path".equalsIgnoreCase(pd.getName()))
                        .forEach(pd -> {
                            SwaggerResource sr = new SwaggerResource();
                            String location = pd.getArgs().get(NameUtils.GENERATED_NAME_PREFIX + "0").replace("/**", API_URI);
